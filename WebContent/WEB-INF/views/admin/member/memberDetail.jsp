...
<script>
	$(document).ready(function() {
		...
		$('#month').on('change', function() {
			$('#day option').remove();
			let selectedDate = new Date($('#year').val(), $('#month').val() - 1);
		...
		$('select[name=mem_sido]').val("${fn:substring(memberInfo.mem_add1, 0, 2) }").attr('selected', 'true');
		$('select[name=mem_gugun]').val("${fn:substring(memberInfo.mem_add1, 2, memberInfo.mem_add1.length() - 1) }").attr('selected', 'true');
...
// 		alert("${fn:substring(memberInfo.mem_add1, 3, memberInfo.mem_add1.length()) }");
		
		$('select[name=mem_sido] option').each(function() {
			if($(this).val() == "${fn:substring(memberInfo.mem_add1, 0, 2) }") {
				$('select[name=mem_gugun]').val("${fn:substring(memberInfo.mem_add1, 3, memberInfo.mem_add1.length()) }").prop('selected', 'true');
			}
		});
		...
		$('form[name=updateMemberForm]').submit(function() {
			...
			$(this).attr('action', '${pageContext.request.contextPath}/admin/member/updateMember.do');
		});
