	...
	<nav id="topbar" role="navigation" style="margin-bottom: 0;" data-step="3" 
		...
		<div class="navbar-header">
			...
			<c:if test="${!empty LOGIN_MEMBERINFO }">
				<a id="logo" href="${pageContext.request.contextPath }/user/join/userMain.do" class="navbar-brand">
					<span class="logo-text text-yellow" style="font-size: 16px;">(재단법인)대덕인재개발원</span>
				</a>
			</c:if>
			<c:if test="${empty LOGIN_MEMBERINFO }">
				<a id="logo" href="${pageContext.request.contextPath }/user/join/loginForm.do" class="navbar-brand">
					<span class="logo-text text-yellow" style="font-size: 16px;">(재단법인)대덕인재개발원</span>
				</a>
			</c:if>
		...
		<c:if test="${LOGIN_MEMBERINFO ne null}">
				...
				<ul class="nav navbar navbar-top-links navbar-right mbn">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="${pageContext.request.contextPath }/user/member/myInfoForm.do?mem_id=${LOGIN_MEMBERINFO.mem_id}"><i class="fa fa-user"></i>프로필관리</a></li>
							...
							<li><a href="${pageContext.request.contextPath }/user/join/signout.do"><i class="fa fa-key"></i>회원탈퇴신청</a></li>
						</ul>
					</li>
				</ul>
			</div>
		</c:if>
		<c:if test="${LOGIN_MEMBERINFO eq null}">
			<div class="loginForm nav navbar navbar-top-links navbar-right"
				...
				<ul>
					...
					<li>&nbsp;</li>	
			        <li class="dropdown">
			        	<button type="button" class="btn btn-warning btn-sm" id="header-login">로그인</button>
				    </li>
				</ul>	    
			</div>
		</c:if>
...
$(document).ready(function() {
	$('#header-login').on('click', function() {
		...
		location.href = '${pageContext.request.contextPath}/user/join/loginCheckHeader.do?mem_id=' + mem_id + "&mem_pass=" + mem_pass;
	});
